...
const useGuestUsage = () => {
  ...
  const fetchUsage = async () => {
    try {
      const response = await fetch('/api/legal/guest/usage');
  ...
  const askGuestQuestion = async (question) => {
    setLoading(true);
    try {
      const response = await fetch('/api/legal/guest/ask', {
        method: 'POST',
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold text-gray-900">Tanya Hukum AI</h1>
            </div>
            ...
            <div className="flex items-center gap-4">
              <div className="text-sm text-gray-600">
                {currentUsage && (
                  <span>
                    {currentUsage.current}/{currentUsage.limit} pertanyaan 
                    {isLoggedIn ? ' hari ini' : ' gratis'}
                  </span>
                )}
              </div>
